...
class GpioGuiNode(Node):
    ...
    def publish_gpio_command(self) -> None:
        msg = DynamicInterfaceGroupValues()
        msg.interface_groups = ['el6910']
        safety_snapshot = self.get_safety_snapshot()

        el6910_value = InterfaceValue()
        el6910_value.interface_names = ['safety_input_vars']
        el6910_value.values = [float(safety_snapshot['safety_input_vars'])]

        msg.interface_values = [el6910_value]
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Layout ── */
    .layout { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; width: 100%; max-width: 960px; }
    .layout .card { max-width: none; }
    @media (max-width: 760px) { .layout { grid-template-columns: 1fr; } }

    /* ── Safety panel ── */
    .safety-card { width: 100%; }
    .s-title { font-size: 1rem; font-weight: 700; margin-bottom: 4px; }
    .s-subtitle { font-size: 0.82rem; color: var(--muted); margin-bottom: 20px; }
    .actions { display: grid; gap: 10px; width: 100%; }
    .action-row {
      display: flex; justify-content: space-between; align-items: center;
      gap: 12px; background: rgba(0,48,110,0.25); border: 1px solid var(--border);
      border-radius: 14px; padding: 12px 14px;
    }
    .action-copy strong { display: block; font-size: 0.9rem; margin-bottom: 2px; }
    .action-copy .meta { color: var(--muted); font-size: 0.78rem; }
    .action-copy code { background: var(--accent-dim); border-radius: 4px; padding: 1px 5px; font-size: 0.78em; color: var(--accent); }
    .sbtn { flex-shrink: 0; border-radius: 999px; padding: 9px 14px; font-size: 0.82rem; font-weight: 700; cursor: pointer; transition: transform 0.12s; }
    .sbtn:hover { transform: translateY(-1px); }
    .sbtn-run { background: rgba(0,196,255,0.14); border: 1px solid var(--accent); color: var(--accent); }
    .sbtn-run.active { background: var(--accent); color: #000c1e; border-color: var(--accent); }
    .sbtn-warn { background: rgba(255,196,0,0.12); border: 1px solid rgba(255,196,0,0.40); color: #ffc400; }
    .sbtn-danger { background: var(--danger-dim); border: 1px solid var(--danger-border); color: var(--danger); }
    .run-badge { display: inline-flex; align-items: center; padding: 4px 10px; border-radius: 999px; font-size: 0.8rem; font-weight: 700; flex-shrink: 0; }
    .run-badge.on  { background: rgba(26,255,136,0.12); color: #1aff88; border: 1px solid rgba(26,255,136,0.30); }
    .run-badge.off { background: var(--danger-dim); color: var(--danger); border: 1px solid var(--danger-border); }
    .s-status-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; width: 100%; margin-top: 14px; }
    .s-status-card { background: rgba(0,48,110,0.25); border: 1px solid var(--border); border-radius: 14px; padding: 12px; }
    .s-card-name { font-size: 0.75rem; font-weight: 600; color: var(--muted); text-transform: uppercase; letter-spacing: 0.07em; }
    .s-card-meta { font-size: 0.70rem; color: var(--muted); margin-top: 1px; }
    .s-card-meta code { background: var(--accent-dim); border-radius: 3px; padding: 1px 4px; font-size: 0.78em; color: var(--accent); }
    .s-card-value { font-size: 1.1rem; font-weight: 800; margin-top: 8px; display: flex; align-items: center; gap: 7px; }
    .s-led { width: 11px; height: 11px; border-radius: 50%; display: inline-block; flex-shrink: 0; }
    .s-led.green { background: #1aff88; box-shadow: 0 0 7px #1aff88; }
    .s-led.red   { background: var(--danger); box-shadow: 0 0 7px var(--danger); }
    .s-footer { margin-top: 12px; color: var(--muted); font-size: 0.72rem; }
  </style>
</head>
<body>
  ...
  <div class="card safety-card">
    <div class="s-title">Safety Controls</div>
    <div class="s-subtitle">TwinSAFE input actions and live safety diagnostics.</div>
    ...
    <div class="actions">
      <div class="action-row">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-shrink:0">
          <span class="run-badge {{ 'on' if safety.run else 'off' }}" id="run-badge">{{ 'ON' if safety.run else 'OFF' }}</span>
          <button class="sbtn sbtn-run{{ ' active' if safety.run else '' }}" id="run-btn" onclick="toggleRun()">Turn {{ 'OFF' if safety.run else 'ON' }}</button>
        </div>
      </div>
      ...
      <div class="action-row">
        ...
        <button class="sbtn sbtn-warn" onclick="pulseSafety(0)">Pulse ACK</button>
      </div>
      ...
      <div class="action-row">
        ...
        <button class="sbtn sbtn-warn" onclick="pulseSafety(2)">Pulse Restart</button>
      </div>

      <div class="action-row">
        <div class="action-copy">
          <strong>Drive Reset</strong>
          <div class="meta">Pulses <code>right_wheel_joint/reset_fault</code> via <code>drive_reset_controller</code>.</div>
        </div>
        <button class="sbtn sbtn-danger" onclick="pulseDriveReset()">Pulse Drive Reset</button>
      </div>
    </div>

    <div class="s-status-grid">
      <div class="s-status-card">
        <div class="s-card-name">ERROR</div>
        <div class="s-card-meta">Bit 0 of <code>safety_output_vars</code></div>
        <div class="s-card-value">
          <span class="s-led {{ 'red' if safety.error_led else 'green' }}" id="error-led"></span>
          <span id="error-text">{{ 'ERROR' if safety.error_led else 'OK' }}</span>
        </div>
      </div>
      <div class="s-status-card">
        <div class="s-card-name">ESTOP</div>
        <div class="s-card-meta">Bit 1 of <code>safety_output_vars</code></div>
        <div class="s-card-value">
          <span class="s-led {{ 'green' if safety.estop_ok else 'red' }}" id="estop-led"></span>
          <span id="estop-text">{{ 'READY' if safety.estop_ok else 'STOPPED' }}</span>
        </div>
      </div>
      <div class="s-status-card">
        <div class="s-card-name">Safety State</div>
        <div class="s-card-meta"><code>safety_logic_state</code></div>
        <div class="s-card-value" id="logic-state-text">{{ safety.logic_state_text }}</div>
      </div>
      <div class="s-status-card">
        <div class="s-card-name">In / Out Vars</div>
        <div class="s-card-meta">Raw bytes for quick checks</div>
        <div class="s-card-value">
          <span id="input-vars-text">{{ safety.safety_input_vars }}</span>&nbsp;/&nbsp;<span id="output-vars-text">{{ safety.safety_output_vars }}</span>
        </div>
      </div>
    </div>

    <div class="s-footer">RUN is maintained. ACK and RESTART pulse high briefly and return low automatically.</div>
  </div>
  ...
  <script>
    ...
    async function poll() {
      try {
        ...
        const s = data.safety;
        if (s) {
          const runOn = Boolean(s.run);
          const runBadge = document.getElementById('run-badge');
          const runBtn   = document.getElementById('run-btn');
          if (runBadge) { runBadge.className = `run-badge ${runOn ? 'on' : 'off'}`; runBadge.textContent = runOn ? 'ON' : 'OFF'; }
          if (runBtn)   { runBtn.textContent = `Turn ${runOn ? 'OFF' : 'ON'}`; runBtn.className = `sbtn sbtn-run${runOn ? ' active' : ''}`; }

          const errLed   = document.getElementById('error-led');
          const errTxt   = document.getElementById('error-text');
          const estopLed = document.getElementById('estop-led');
          const estopTxt = document.getElementById('estop-text');
          const lsTxt    = document.getElementById('logic-state-text');
          const inVars   = document.getElementById('input-vars-text');
          const outVars  = document.getElementById('output-vars-text');

          const err = Boolean(s.error_led);
          if (errLed)   { errLed.className   = `s-led ${err ? 'red' : 'green'}`; }
          if (errTxt)   { errTxt.textContent   = err ? 'ERROR' : 'OK'; }
          const ok = Boolean(s.estop_ok);
          if (estopLed) { estopLed.className = `s-led ${ok ? 'green' : 'red'}`; }
          if (estopTxt) { estopTxt.textContent = ok ? 'READY' : 'STOPPED'; }
          if (lsTxt)    { lsTxt.textContent    = s.logic_state_text; }
          if (inVars)   { inVars.textContent   = s.safety_input_vars; }
          if (outVars)  { outVars.textContent  = s.safety_output_vars; }
        }
      } catch (_) {}
    }
    ...
    async function toggleRun() {
      try {
        await fetch('/set_run', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
      } catch (_) {}
    }

    async function pulseSafety(bit) {
      try {
        await fetch('/pulse_safety', {
          method: 'POST',
    ...
    async function pulseDriveReset() {
      try {
        await fetch('/pulse_drive_reset', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
      } catch (_) {}
    }
